...
const FileUploader: React.FC<FileUploaderProps> = ({
  ...
  const validateFile = (file: File) => {
    ...
    if (!fileType.match(/(image\/jpeg|image\/png|application\/pdf)/)) {
    ...
    if (file.size > maxFileSize) {
      toast.error(`File size exceeds the maximum limit of ${maxFileSize / (1024 * 1024)}MB.`);
      return false;
    }
    ...
    <div className="flex items-center justify-center w-full h-full">
      <div 
        className={cn(
          ...
          isUploading ? "opacity-70 pointer-events-none" : "hover:border-primary/50 hover:bg-muted/50"
        )}
      ...
      >
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            Max file size: {maxFileSize / (1024 * 1024)}MB<br />
